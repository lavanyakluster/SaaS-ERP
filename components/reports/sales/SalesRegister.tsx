...
import type { SalesRegisterRecord } from '@/lib/types/sales-register.types';
...
export default function SalesRegister() {
  ...
  useEffect(() => {
    ...
    console.log('🔐 Token Status on SalesRegister mount:', {
      hasToken: !!token,
  ...
  const { data: salesData, isLoading, error } = useSalesRegister({
  ...
  useEffect(() => {
    console.log('📊 Sales Register Data:', {
      data: salesData,
  ...
  }, [salesData, isLoading, error, filters]);

  const handleLoadReport = (newFilters: ReportFilters) => {
    console.log('🔄 Loading Sales Register with filters:', newFilters);
  ...
  const groupedData = useMemo(() => {
    ...
    const groups: Record<string, SalesRegisterRecord[]> = {};
  ...
  const grandTotal = useMemo(() => {
    ...
    return salesData.reduce((sum, record) => sum + record.Net, 0);
  }, [salesData]);
      ...
      {isLoading ? (
        <div className={`rounded-lg border p-8 text-center ${
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <p className={isDark ? 'text-gray-300' : 'text-gray-700'}>Loading sales register...</p>
          </div>
        </div>
      ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-indigo-600 text-white sticky top-0 z-10">
                      <tr>
                        ...
                        <th className="text-left p-3 font-semibold whitespace-nowrap">LPO No</th>
                        <th className="text-left p-3 font-semibold whitespace-nowrap">LPO Date</th>
          ...
          {(!salesData || salesData.length === 0) && !isLoading && (
            <div className={`rounded-lg border p-8 text-center ${
              ...
              <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>
                No sales register data available for the selected filters
              </p>
            </div>
          )}
        </div>
...
function MasterRow({
  ...
  const { data: detailData, isLoading: isLoadingDetail } = useSalesRegisterDetail(
    {
      shid: record.ID,
      brCode: record.Branch.replace('P', '').replace('V', '').replace('M', '').replace('A', ''),
    },
    isExpanded
  );
  ...
  return (
    <>
      ...
      {isExpanded && (
        <tr>
          <td colSpan={9} className={`p-0 ${isDark ? 'bg-gray-900' : 'bg-gray-100'}`}>
            <div className="p-4">
              {isLoadingDetail ? (
